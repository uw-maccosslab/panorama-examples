...
if not args.url:
    logger.error("A url to a Panorama server must be specified on the command line")
    parser.print_usage()
    sys.exit(1)
if not args.folder:
    logger.error("Folder on Panorama server must be specified on the command line")
    parser.print_usage()
    sys.exit(1)
if not args.file_name:
    logger.error("The file to be uploaded must be specified on the command line")
    parser.print_usage()
    sys.exit(1)
if args.debug:
...
csrf_url = "{}/login/{}/whoami.api".format(args.url, args.folder)
try:
    ...
    logger.error("Authenticating to the server failed with Http Error %s", errh)
    sys.exit(1)
except requests.exceptions.ConnectionError as errc:
    logger.error(
        "Authenticating to the server Download failed with Error Connecting %s", errc
    )
    sys.exit(1)
except requests.exceptions.Timeout as errt:
    logger.error(
        "Authenticating to the server Download failed with Timeout Error %s", errt
    )
    sys.exit(1)
except requests.exceptions.RequestException as err:
    logger.error("Authenticating to the server Download failed with error %s", err)
    sys.exit(1)
...
url = "{}/_webdav/{}/%40files/{}/?Accept=application/json&overwrite=T&X-LABKEY-CSRF={}".format(
    args.url, args.folder, args.experiment, csrf_header
)

# Create the dictionary containing the file name and other file attributes
files = {
    "file": (
        args.file_name.split("/")[-1:][0],
        open(args.file_name, "rb"),
...
# Upload the file
logger.info("Upload %s to %s folder on %s", args.file_name, args.folder, args.url)
try:
    # Upload the file
    ...
    if upload_response.status_code == 207:
        ...
        logger.error("Upload of %s has failed", args.file_name)
        logger.error(
